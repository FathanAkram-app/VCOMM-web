...
import { getMessages, addMessage } from "../utils/chatUtil";
...
export default function ChatRoom({ chatId, isRoom, chatName, onBack, currentUserId, currentUsername, onNavigateToChat, forwardedMessage }: ChatRoomProps) {
  // Fix TypeScript errors
  let recipientMessages: any[] = [];
  console.log("ChatRoom rendered with:", { chatId, isRoom, chatName, currentUserId, currentUsername });
  const { user } = useAuth();
  
  // Use currentUserId and currentUsername as fallback when useAuth fails
  const effectiveUser = user || { id: currentUserId, username: currentUsername };
  
  // Debugging information untuk membantu troubleshooting
  useEffect(() => {
    console.log("ChatRoom component mounted with ID:", chatId);
    console.log("Is Room Chat:", isRoom);
  ...
  // Initialize with empty array first, then load from localStorage
  ...
  // Load messages from localStorage when component mounts using the new chatUtil
  useEffect(() => {
    try {
      // Ambil pesan dari penyimpanan chat
      const storedMessages = getMessages(chatId, isRoom);
      
      if (storedMessages && storedMessages.length > 0) {
        console.log(`Loaded ${storedMessages.length} messages for chat ${chatId}`);
        
        // Konversi format pesan untuk UI
        const formattedMessages = storedMessages.map((msg: any) => ({
          id: msg.id,
          content: msg.text,
          sender: {
            id: msg.senderId,
        ...
        // Jika tidak ada pesan tersimpan, gunakan data awal
        console.log(`No stored messages found for chat ${chatId}, using initial data`);
        const initialMessages = getInitialMessages();
        setMessages(initialMessages);
        
        // Format data untuk penyimpanan dan simpan ke penyimpanan global
        const messagesToStore = initialMessages.map(msg => ({
          id: msg.id,
        ...
        // Simpan pesan awal ke penyimpanan chat sederhana
        messagesToStore.forEach(msg => {
          addMessage(chatId, isRoom, msg);
        });
        
        console.log(`Initial messages saved for chat ${chatId}`);
      }
    } catch (error) {
      console.error('Error loading messages:', error);
      // Fallback ke data awal jika terjadi error
  ...
  const handleSendMessage = () => {
    ...
    if (message.trim() || attachments.length > 0) {
      ...
      // Simpan pesan ke penyimpanan global
      try {
        // Format pesan baru untuk disimpan
        ...
        // Tambahkan pesan ke penyimpanan global menggunakan sistem simpleChat
        addMessage(chatId, isRoom, messageToStore);
        
        console.log(`Message saved to global chat storage for chat ID ${chatId}`);
      } catch (error) {
        console.error('Error saving message to global storage:', error);
      }
      
      // If we have a WebSocket connection, send the message to the server
      try {
        ...
        // Try to get WebSocket from window object if available
        const chatWs = (window as any).chatWebSocket;
        
        if (chatWs && chatWs.readyState === WebSocket.OPEN) {
          chatWs.send(JSON.stringify(messageData));
          console.log('Message sent via WebSocket:', messageData);
        } else {
          // Fallback if WebSocket not available or not connected
          console.log('WebSocket not available, message already saved to localStorage');
        }
      } catch (error) {
