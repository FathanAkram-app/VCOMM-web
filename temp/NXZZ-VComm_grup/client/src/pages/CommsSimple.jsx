...
import { initializeChats, getUserChats } from '../lib/sharedChat';

// Komponen simple untuk menampilkan daftar chat
class CommsSimple extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      chats: [],
      loading: true,
      currentUser: { id: 9 } // Default ke User Aji
    };
  }
  ...
  componentDidMount() {
    ...
    // Coba load user data dari lokal storage
    try {
      const authStr = localStorage.getItem('authCredentials');
      if (authStr) {
  ...
  // Load daftar chat untuk user saat ini
  loadUserChats() {
    ...
    this.setState({ chats: userChats, loading: false });
  }
    ...
    return (
      <div className="flex flex-col h-screen bg-[#1f201c]">
              ...
              {chats.map(chat => (
                ...
                >
                  ...
                  <div className="relative mr-3">
                    <div className={`h-12 w-12 ${chat.isRoom ? 'bg-[#566c57]' : 'bg-[#3d5a65]'} rounded-full flex items-center justify-center text-white font-medium`}>
        ...
        <div className="h-16 bg-[#2a2b25] border-t border-[#3d3f35] flex justify-around items-center">
          ...
          <div className="flex flex-col items-center text-[#bdc1c0] hover:bg-[#3d3f35] hover:text-white px-4 py-2 rounded-md" onClick={() => this.goToNewChat()}>
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
