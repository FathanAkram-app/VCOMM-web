...
export default function WhatsAppStyleChatList({
  ...
  // Load chat list dari prop chats jika tersedia
  useEffect(() => {
    if (chats && chats.length > 0) {
      console.log("Using provided chats:", chats);
      setChatItems(chats);
      setIsLoadingChats(false);
      return;
    }
    
    // Fallback ke hardcoded chats jika tidak ada
    const userData = getAuthData();
    const userId = userData?.id;
    
    if (userId) {
      console.log("Fallback: Using hardcoded chats for user ID:", userId);
      
          // Untuk pengguna baru atau semua pengguna, tampilkan daftar chat kosong
      setChatItems([]); // Tidak menampilkan chat default untuk pengguna baru
    }
  ...
  // Deduplikasi chat items secara manual
  // Pertama, ambil chat room (group)
  const roomChats = chatItems.filter(chat => chat.isRoom);
  ...
  // Filter direct chats
  chatItems.filter(chat => !chat.isRoom).forEach(chat => {
  ...
  const finalChatList = [...roomChats, ...uniqueDirectChats];
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 bg-[#1e1e1e] border-[#3d5040] text-[#e4e6e3]">
                  <DropdownMenuItem 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      // Masukkan ke dalam grup (implementasi dummy)
                      alert(`${chat.name} akan dimasukkan ke dalam grup baru`);
                    }}
                  >
                  ...
                  <DropdownMenuItem 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      // Feature: Membuat grup dengan kontak ini
                      alert(`Membuat grup baru bersama ${chat.name}`);
                    }}
                  >
                    <MessageSquare className="mr-2 h-4 w-4" />
                    Buat Grup Baru
                  ...
                  <DropdownMenuItem 
                    ...
                    onClick={(e) => {
                      ...
                      if (confirm(`Apakah Anda yakin ingin membersihkan riwayat chat dengan ${chat.name}? Semua pesan akan dihapus.`)) {
                        // Panggil fungsi untuk membersihkan chat
                        if (onClearChatHistory) {
                          onClearChatHistory(chat.id, chat.isRoom);
                        } else {
                          alert("Fungsi pembersihan chat belum diimplementasikan");
                        }
                      }
                    }}
                  >
                  ...
                  <DropdownMenuItem 
                    ...
                    onClick={(e) => {
                      ...
                      if (confirm(`Apakah Anda yakin ingin menghapus chat dengan ${chat.name}?`)) {
                        handleDeleteChat(chat.id, chat.isRoom, e as any);
                      }
                    }}
                  >
                    <Trash className="mr-2 h-4 w-4" />
                    Hapus {chat.isRoom ? "Group" : "Chat"}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
